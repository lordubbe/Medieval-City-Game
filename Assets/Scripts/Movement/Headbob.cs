...
public class Headbob : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetAxisRaw("Horizontal") != 0 || Input.GetAxisRaw("Vertical") != 0) //moving
        {
            ...
            Vector3 newPosition = camPos + new Vector3(Mathf.Cos(timer) * bobAmount, restPosition.y + Mathf.Abs((Mathf.Sin(timer) * bobAmount)), restPosition.z); //abs val of y for a parabolic path
            transform.localPosition = newPosition;
        }
        else
        {
            timer = 0; //reinitialize
